...
public class AuthService {
    ...
    private void issueOtp(User user, boolean reset) { String code = otp(); Instant now = Instant.now(); if (reset) { user.setResetOtp(code); user.setResetOtpExpiry(now.plus(otpExpiryMinutes, ChronoUnit.MINUTES)); user.setResetOtpSentAt(now); } else { user.setOtp(code); user.setOtpExpiry(now.plus(otpExpiryMinutes, ChronoUnit.MINUTES)); user.setOtpSentAt(now); } users.save(user); email.sendOtp(user.getEmail(), code, reset); }
    ...
    @Transactional public MessageResponse verify(VerifyOtpRequest request) { User user = find(request.email()); if (user.isVerified()) throw new ApiException(HttpStatus.CONFLICT, "Email is already verified."); if (user.getOtp() == null || !user.getOtp().equals(request.otp())) throw new ApiException(HttpStatus.BAD_REQUEST, "Invalid OTP."); if (user.getOtpExpiry() == null || user.getOtpExpiry().isBefore(Instant.now())) throw new ApiException(HttpStatus.BAD_REQUEST, "OTP has expired."); user.setVerified(true); user.setOtp(null); user.setOtpExpiry(null); user.setOtpSentAt(null); users.save(user); return new MessageResponse("Email verified successfully."); }
    ...
    @Transactional public LoginResponse login(LoginRequest request) { User user = find(request.email()); if (!encoder.matches(request.password(), user.getPassword())) throw new ApiException(HttpStatus.UNAUTHORIZED, "Invalid email or password."); if (!user.isVerified()) throw new ApiException(HttpStatus.FORBIDDEN, "Please verify your email first."); return new LoginResponse(jwt.generate(user.getEmail()), toResponse(user)); }
    @Transactional public MessageResponse forgot(ForgotPasswordRequest request) { User user = find(request.email()); checkCooldown(user.getResetOtpSentAt()); issueOtp(user, true); return new MessageResponse("Password reset OTP sent to your email."); }
    @Transactional public MessageResponse reset(ResetPasswordRequest request) { User user = find(request.email()); if (user.getResetOtp() == null || !user.getResetOtp().equals(request.otp())) throw new ApiException(HttpStatus.BAD_REQUEST, "Invalid reset OTP."); if (user.getResetOtpExpiry() == null || user.getResetOtpExpiry().isBefore(Instant.now())) throw new ApiException(HttpStatus.BAD_REQUEST, "Reset OTP has expired."); user.setPassword(encoder.encode(request.newPassword())); user.setResetOtp(null); user.setResetOtpExpiry(null); user.setResetOtpSentAt(null); users.save(user); return new MessageResponse("Password updated successfully."); }
